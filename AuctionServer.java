  ...
  public static void main(String args[]) {
    ...
    while (true) {
		...
		try	{
		  ...
		  if (if2SellerConnected()) {
		  //refuse connection if 2 sellers are already connected
		  ...
		  //add seller to first empty thread - max 7
			  for (int i = 0; i < maxThreadCount; i++) {
				if (clientThreads[i] == null) {
	...
	private static boolean if2SellerConnected() {
		int auxiliar = 0;
		for (int i = 0; i < maxThreadCount; i++) {
			if(clientThreads[i] != null) {
...
class clientThread extends Thread {
  ...
  public LinkedList<AuctionItem> auctionList;
  private LinkedList<Message> messageQueue;
  private volatile AuctionItem confirmationItem; // To store the item awaiting confirmation
  private volatile Timer confirmationTimer = new Timer(); // Timer for confirmation timeout
  ...
  public void run() {
    ...
    try {
      ...
      while (true) {
        ...
        else if(line.startsWith("listbids")) {
        	if(!isSeller) {
				System.out.println("Buyer (" + name + ") attempted to call LISTBID function: " + line);
				this.os.println("> Buyer client cannot list bids");
			}
			else
				listbids(line);
        	}
  ...
  boolean login(String input)
  {
	...
	try {
		...
		if(isSeller && checkLogin(loginName[1]) || !isSeller && checkLogin(loginName[1])) {
			synchronized (this) {
  ...
  void add(String input) {
	    String[] element = input.split(" ", 3); // Split into 3 parts to capture item number and item name

	    try {
	        System.out.println(name + ": sent add command: " + input);

	        synchronized (this) {
	            AuctionItem newItem = new AuctionItem(Integer.parseInt(element[1]), element[2], name); // Use the seller's name from OS login
	            ...
	            while (iterator.hasNext()) {
	                temp = iterator.next();
	                if (temp.getItemNumber() == newItem.getItemNumber()) {
	            ...
	            if (!found) {
	                ...
	                auctionList.add(newItem);
	                this.os.println("> Added " + newItem.getItemNumber() + "/" + newItem.getItemName() + " to Auction List");
	                System.out.println(name + ": added " + newItem.getItemNumber() + "/" + newItem.getItemName() + " to Auction List");
	            } else {
	...
	if(auctionList.size() == 0)
		...
		try {
			...
			while(iterator.hasNext()) {
				...
				temp = iterator.next();
				this.os.print("Item Number: " + temp.getItemNumber() + "  Item Name: " + temp.getItemName() + "  Highest Bid: " + temp.getHighestBid() + "Seller name: " + temp.getSellerName() + "  Highest Bidder: ");
	            ...
	            if (auctionList.size() == 0) {
	                ...
	                while (iterator.hasNext()) {
	                    ...
	                    if (temp.getItemNumber() == Integer.parseInt(element[1])) {
	                        ...
	                        // Check if the user sending the command is the seller of the item
	                        if (!temp.getSellerName().equalsIgnoreCase(name)) {
	                            this.os.println("> You are not the seller of item " + temp.getItemNumber());
	                            return;
	                        }

	                        // If the item number is found, list all bids for that item
	                        found = true;
	                        List<Bid> bids = temp.printLista();
	                        this.os.println("List of bids for item " + temp.getItemName());
	                        for (Bid bid : bids) {
	                            this.os.println(bid.getNome() + " " + bid.getValor());
	                        }
	                    }
	                }
				...
				while(iterator.hasNext()) {
					...
					if (temp.getItemNumber() == Integer.parseInt(element[1])) {
						...
						if(temp.getHighestBidder() == null || temp.getHighestBid() < Integer.parseInt(element[2])) {
							...
							//udpate highest bidder + amount for item
							...
							//create a index in controller so we can map the list of bids and its items
							//Bid aux = new Bid(name, Integer.parseInt(element[2]));
							//BidItemList.add(temp, aux);
							temp.addBid(name, Integer.parseInt(element[2]));
  ...
  void sell(String input) {
	    try {
	        ...
	        try {
	            ...
	            synchronized (this) {
	                ...
	                while (iterator.hasNext()) {
	                    ...
	                    if (temp.getItemNumber() == Integer.parseInt(element[1])) {
	                        ...
	                        // Remove the item from the list before confirmation
	                        auctionList.remove(temp);

	                        // Store the item for confirmation
	                        setConfirmationItem(temp);

	                        // Start the confirmation timer (1 minute timeout)  
	                        confirmationTimer.schedule(new PaymentConfirmationTask(this), 60_000);

	                        // Notify the top bidder to confirm payment
	                        String message = "> You won the auction for item " + temp.getItemNumber() +
	                                ". Please confirm your payment by typing 'confirm'.";
	                        sendMessages(temp, message);

	                        this.os.println("> Seller confirmation sent to the top bidder.");
	                        System.out.println(name + ": Seller confirmation sent for item: " + temp.getItemNumber());
	            ...
	            if (!itemFound) {
	                this.os.println("> Item number " + element[1] + " does not exist in the list");
  ...
  void confirmPayment() {
	    synchronized (this) {
	        AuctionItem confirmedItem = getConfirmationItem();
	        if (confirmationTimer != null && confirmedItem != null) {
	            // Cancel the confirmation timer
	            confirmationTimer.cancel();
	            confirmationTimer = null;

	            // Complete the transaction
	            confirmedItem.setPaymentApproved(true);
	            this.os.println("> Payment confirmed. Transaction completed for item " + confirmedItem.getItemNumber());
	            System.out.println(name + ": Payment confirmed for item: " + confirmedItem.getItemNumber());

	            // Reset the confirmation item
	            setConfirmationItem(null);
	        } else {
	            System.out.println("Confirmation timer or item is unexpectedly null.");
	        }
	    }
	}
  ...
  // returns index in arraylist if recipient string is connected
	...
	boolean checkLogin(String input) {
		String[] list = {"Thomaz", "Cinthia", "Daniel", "Lucas", "Marcio", "Roberto"};
	...
	public AuctionItem getConfirmationItem() {
        synchronized (this) {
            return confirmationItem;
        }
    }

    public void setConfirmationItem(AuctionItem item) {
        synchronized (this) {
...
class PaymentConfirmationTask extends TimerTask {
    private final clientThread client;

    public PaymentConfirmationTask(clientThread client) {
        this.client = client;
    }

    @Override
    public void run() {
        // Timeout occurred, notify the client
        client.os.println("> Payment confirmation timeout. Item will be re-listed.");
        // Add the item back to the auction list
        client.auctionList.add(client.getConfirmationItem());
    }
=======
// Andrew Shaffer (G00688109)
